...
const set_session = (username, res, session_id) => {
  ...
  // optional param to use existing sessions
  if (session_id) {
  ...
  pool.query(
    ...
    (q0_err, q0_res) => {
      ...
      if (q0_res.rows.length === 0) {
        ...
        res
          .status(409)
          .json({ type: "error", msg: "This username has been taken" })
      }
    }
  )
})
  ...
  pool.query(
    ...
    (q_err, q_res) => {
      ...
      if (user && user.password_hash === hash(password)) {
        ...
        res
          .status(400)
          .json({ type: "error", msg: "Incorrect username/password" })
      }
    }
  )
})
...
router.get("/authenticated", (req, res, next) => {
  ...
  pool.query(
    ...
    (q_err, q_res) => {
      if (q_err) return next(q_err)
      res.json({ authenticated: Session.verify(req.cookies.session_str) &&
